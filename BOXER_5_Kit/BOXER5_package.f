...
c
c	Authors:
c	Vannucci and Gasperini (2024)
c
c	gianfranco.vannucci@ingv.it
c	paolo.gasperini@unibo.it
c
c	Notes: gfortran usable for Winsows, Mac, Unix, Linux systems 
c
c***********************************************************************
c*SUB EPCNTR  **********************************************************
c***********************************************************************
      subroutine epcntr(n,xla,xlo,fint,ival,xlaep,xloep,xlaerd,xloerd,
     1xlaer,xloer,fin0,finm,npe,xlat,xlon,iout,locali,ifout4)
c
...
c	ifout4: Fortran unit number of output file
c	
c	Output:
c 
...
c
      ...
      if(iout.ge.2)then
         write(ifout4,'(/,1x,a,/)')'Used data to compute epicenter:'
         write(ifout4,'(1x,a)')'    Lat     Lon    I  locality'
         do i=1,mm
             write(ifout4,'(1x,2f9.4,1x,f5.1,1x,a)')xlat(i),xlon(i),
     1       fin(i),loca(i)
         enddo
         write(ifout4,'(1x)')
      endif
...
c***********************************************************************
     ...
     1aai,bbi,cci,azi1,azi2,ranlat,ranlon,depmax,ifout4)
c
c	compute macroseismic epicenter, depth, intensity at epicenter and attenuation 
c	coefficients using attenuation function (e.g. Pasolini et al. (2008b))
c
c	Input:
c
...
c	amg: intercept of M-Io relationship (eg Pasolini et al., 2008b)
c	bmg: coefficient of M-Io relationship (eg Pasolini et al., 2008b)
c	aattcoe: coefficient a (linear term) of attenuation equation (eg Pasolini et al., 2008b)
c	battcoe: coefficient b (log term) of attenuation equation (eg Pasolini et al., 2008b)
c	depref: initial depth (km) (eg Pasolini et al., 2008b)
...
c
      implicit doubleprecision (a-h,o-z)
      parameter(nep=6)
c	parameter(npts=4500,lp=7)
	parameter(npts=2500,lp=7)
      ...
      doubleprecision maxv
      character (len=20) locali(*),cloc(npts),localimin
      logical iexit
      character (len=11) lab(lp)
	character (len=5) lab1(lp)
     ...
     1,twopi/6.283185307d0/
      ...
      iexit=.false.
      npun=0
      write(ifout4,'('' Attenuation function location method '')')
      if(nincin.eq.1)then
        write(ifout4,'('' 2 free parameters (lat,lon)    '')')
       ninc=2
      else if(nincin.eq.2)then
        write(ifout4,'('' 3 free parameters (lat,lon,dep)'')')
        ninc=3
      else if(nincin.eq.3)then
        write(ifout4,'('' 3 free parameters (lat,lon,Ie) '')')
      else if(nincin.eq.4)then
        write(ifout4,'('' 4 free parameters (lat,lon,Ie,dep) '')')
      else if(nincin.eq.5)then
        write(ifout4,'('' 5 free param.(lat,lon,Ie,dep,lin) '')')
      else if(nincin.eq.6)then
        write(ifout4,'('' 6 free param.(lat,lon,Ie,dep,lin,log) '')')
      endif
      write(ifout4,*)
      do i=1,n
         if(dmod(fint(i),0.5d0).lt.1.d-4)then
            ddis=distgeod(xlaep,xloep,xla(i),xlo(i))
            ddi=diex_bili(ddis)
c
...
c
      call errors(nincin,ninc,iout,ifout4,x,var,xlaer,xloer,
...
c
	...
	1xdeper.ge.0.d0)then
         call ellips(nincin,ninc,iout,ifout4,var,xlaep,
...
c
	...
	1xdeper.ge.0.d0)then
         nincinX2D=3
         nincX2D=3
         call ellips(nincinX2D,nincX2D,iout,ifout4,var,xlaep,
...
c
      is=2
      if(iout.ge.4)then
         write(ifout4,*)'Data used to locate epi/hypocenter'
         write(ifout4,*)
         write(ifout4,*)'Locality                Dist Azi  '//
     1   'Lat       Lon       Int  Res   Mag   Wgt'
      endif
      ddismin=100000.d0
      sumres=0.d0
      do npu=1,npun
         ddisv(npu)=distgeod(xlaep,xloep,xlat(npu),xlon(npu))
...
c
      ...
      do np=1,npun
         ...
         sumres=sumres+resv(npu)
         if(iout.ge.4)then
            write(ifout4,'(1x,a20,1x,F6.1,1x,i3,2(1x,f9.4),1x,f5.1,
...
c	aziv(npun+1)=aziv(1)+360.d0
	aziv(npun+1)=aziv(1)
      ...
      npe=npun
124   return
125   write(ifout4,'(/,'' N data low            : '',i5,
...
c
      parameter(lp=7)
      parameter(lwk=2*lp*(2*lp+8)+lp)
      common/metlik/met
      dimension xguess(lp),xlb(lp),xub(lp),xscale(lp),iparam(7)
     1,rparam(7),x(lp),gc(lp),wk(lwk),iwk(lp)
      integer ifilein1
...
c
      xguess(1)=xlaep
      xguess(2)=xloep
      xlb(1)=xguess(1)-ranlat ! lat
      xlb(2)=xguess(2)-ranlon ! lon
      xub(1)=xguess(1)+ranlat ! lat
      xub(2)=xguess(2)+ranlon ! lon
c
      if(nincin.ge.3)then
         xguess(3)=fin0
         xlb(3)=-1.d0
...
c	IPARAM(2) Number of good digits in the function. Default: Machine dependent
	iparam(2)=0 ! avoid errors (e.g. sqrt domain error).Differences for some values after 3th decimal. gf 28/03/2022
...
c	write(*,*)'xguess',xguess
c	write(*,*)'ibtype',ibtype
c	write(*,*)'xlb,xub',xlb,xub
c	write(*,*)'xscale',xscale
c	write(*,*)'fscale',fscale
c	write(*,*)'iparam',iparam
c	call du4inf(iparam,rparam)
c	write(*,*)'rparam',rparam
      
      if(nincin.ge.3)then
c	write(*,*)'nincin',nincin,ninc1
c	   call dbconf(likel_sim6,ninc1,xguess,ibtype,xlb,xub,
c	1   xscale,fscale,iparam,rparam,x,fvalue)
	   call db2onf(likel_sim6,ninc1,xguess,ibtype,xlb,xub,
	1   xscale,fscale,iparam,rparam,x,fvalue,wk,iwk)
c	   call dbcong(likel_sim6,grad,ninc1,xguess,ibtype,xlb,xub,
c	1   xscale,fscale,iparam,rparam,x,fvalue)
      else
c	call likel_simi(ninc1,xguess,aaa) ! test for problem on likel_simi
c	write(*,*)'fffff',aaa
c	   call dbconf(likel_simi,ninc1,xguess,ibtype,xlb,xub,
c	1   xscale,fscale,iparam,rparam,x,fvalue)
	   call db2onf(likel_simi,ninc1,xguess,ibtype,xlb,xub,
	1   xscale,fscale,iparam,rparam,x,fvalue,wk,iwk)
c	   call dbcong(likel_simi,grad,ninc1,xguess,ibtype,xlb,xub,
c	1   xscale,fscale,iparam,rparam,x,fvalue)
      endif
c	14/04/2019 gf add comment (i.e. erase) icod and iertp of previous versions
c	icod=iercd()
c	iertp=n1rty(1)
c
      return
...
c*SUB ERRORS  *********************************************************
c***********************************************************************
      subroutine errors(nincin,ninc,iout,ifout4,x,var,xlaer,
...
c
	...
	eps=0.d0
      ...
      enddo
c
c	call dlinrg(ninc1,h,lp,var,lp)
	call dl2nrg(ninc1,h,lp,var,lp,wk,iwk)
c
     ...
     1xdeper.lt.0.d0)then
         if(iout.ge.2)write(ifout4,'('' Hessian matrix error  : '')')
      endif
c
...
c
      if(iout.ge.2)then
         if(iout.ge.4)then
            write(ifout4,'(/,'' Hessian matrix'')')
            call matrixwr(nincin,ninc,ninc1,lab,lab1,h,ifout4)
c
            write(ifout4,'(/,'' Variance/covariance matrix'')')
            call matrixwr(nincin,ninc,ninc1,lab,lab1,var,ifout4)
            nmet=ninc
     ...
     1      (ninc.le.6.and.var(nmet,nmet).ge.0.d0))then
               ...
               enddo
c
               write(ifout4,'(/,'' Correlation matrix'')')
               call matrixwr(nincin,ninc,ninc1,lab,lab1,cor,ifout4)
c
            endif
            write(ifout4,*)
         endif
...
c*SUB ELLIPS  **********************************************************
c***********************************************************************
      subroutine ellips(nincin,ninc,iout,ifout4,var,xlaep,ermax,azmax
...
c
      ...
      if(iout.ge.4)then
         write(ifout4,'(/,'' Ellipse Variance/covariance matrix'')')
         if(numaut.le.2)then
            write(ifout4,'(6a11)')lab(1),lab(2)
         else
            write(ifout4,'(6a11)')lab(1),lab(2),lab(4)
         endif
         do k=1,2
            write(ifout4,'(a5,6(1x,g10.4))')lab1(k),(var(k,j)
     1      ,j=1,numaut)
         enddo
         if(numaut.eq.3)write(ifout4,'(a5,6(1x,g10.4))') 
     1   lab1(4),(var(k,j),j=1,numaut)
         write(ifout4,*)
      endif
c      call devcsf(numaut,var1,lp,eval,evec,lp)
      call de5csf(numaut,var1,lp,eval,evec,lp,wk,iwk)
      if(numaut.eq.2)then
...
c
c	compute expected intensity decrement using log linear law (Pasolini et al., 2008b)
c
      implicit doubleprecision (a-h,o-z)
...
c
      ...
      do i=1,npun
         ...
         iossl=aioss+0.01d0
         ddis=distgeod(x(1),x(2),xlat(i),xlon(i))
...
c	write(*,*)'pp',i,npun,anou,anol,f,sigma,dlog(sigma),
c	1-npun*dlog(sigma)
c	14/04/2019 gf add
	   if(f.lt.-1.d299)f=-1.d10
      enddo
      f=-(f-npun*dlog(sigma))
c
      return
...
c	14/04/2019 gf change 300 -> 299
c	   if(f.lt.-1.d300)f=-1.d10
	   if(f.lt.-1.d299)f=-1.d10
c	write(*,*)'pp',i,npun,anou,anol,f,sigma,dlog(sigma),
c	1-npun*dlog(sigma)
      enddo
...
c*SUB DISGTEO  *********************************************************
c***********************************************************************
      doubleprecision function distgeod(xlat1,xlon1,xlat2,xlon2)
c
...
c
     ...
     1*dcos(xlat1*dtor)*dcos((xlon1-xlon2)*dtor)
      ...
      distgeod=dacos(arg)*ar
      return
      end
c***********************************************************************
c*SUB DISTGEOd3D  ******************************************************
c***********************************************************************
      real*8 function distgeod3D
	1(xlat1,xlon1,xlat2,xlon2,depth2)
c
c	compute 3D geodetic distance (in km) for a spherical earth
c
...
c	dtor=pig/180.d0
	dtor=datan(1.d0)/45.d0
      rtod=180.d0/pig
c
	dist=distgeod(xlat1,xlon1,xlat2,xlon2)
      ddisd=dsqrt(dist**2+depth2**2)
	ard=ar-dabs(depth2)
	angp=dist/ar
c	angc=90.d0*dtor-ang
	distp=dsqrt(ar**2.d0+ard**2.d0-2.d0*ar*ard*dcos(angp))
c	distc=ar*dcos(angc)
	distgeod3D=distp
	if(distgeod3D.lt.dist)then
c	   angz=dacos(ard/ar)
	   angh=dacos(depth2/dist)
c	   distz=ar*dsin(angz)
	   disth=dist*dsin(angh)
c	   write(*,*)'bbbb',distz,disth,angh*rtod
	   distgeod3D=disth
	endif
c	write(*,*)'values',distp,disth
c	write(*,*)'distgeod3D',distgeod3D,ddisd,distgeod3D-ddisd
c
      return
      end
c***********************************************************************
...
c***********************************************************************
     ...
     1nok,iout,aiv,stda,awme,spes,ifout4)
c
...
c	gf 11/12/2018: to avoid erro=0, due to Nan for stda, awme in inpparm
         if(ndat(ll).ne.0.and.awme(ll).gt.0.d0.and.stda(ll).gt.0.d0)then
            xint=fintg
            niq(ll)=ndat(ll)
            rva(ll)=rmedn(ll)
c
     ...
     1        '  I      R      area     N  M     Wgt'
            do l=1,nrag
               if(va(l).ne.0.d0)then 
                  write(ifout4,'(1x,f5.1,1x,f6.1,1x,f8.0,1x,i4,1x,
...
c
         ...
         if(nok.gt.1.and.iout.ge.2)then
            write(ifout4,'(/,1x,a,1x,3f5.2)')
     1      'Mean and std          :',amean,s,smed
            write(ifout4,'(1x,a,1x,f5.2)')
     1      'Median                :',ame
         endif
c
...
c
         ...
         if(nok.ge.4)then
            if(iout.ge.2)then
               write(ifout4,'(/,1x,a,/)')'Trimmed mean used '
               write(ifout4,'(1x,a,1x,15f7.2)')
     1           'M  ',(va2(l),l=2,nok-1)
               write(ifout4,'(1x,a,1x,15f7.2)')
...
c
     ...
     1         (1.d0/erro1(iper(l))**2-1.d0)*iwgt)
            ...
            if(antot.gt.0.d0)spes=dsqrt(1.d0/antot)
         else
c
...
c***********************************************************************
     ...
     1,ncldis,nok2,iout,ifout4,iselp,aai,bbi,cci)
c
...
c	 13/12/2018 gf
c	parameter(npom=4500)
	parameter(npom=2500)
      dimension raiw(*),fint(*),aint0v(npom),wgt(npom),fintv(npom),
     1raiwv(npom),work(npom)
      aa=aai
      bb=bbi
      cc=cci
...
c
c	discard MDPs at long distances if iselp.ne.2 
c
...
c*
      ...
      if(iout.ge.5)then
	   write(ifout4,'(/,a,/)')
	1   'Data used to compute magnitude (ATN)'
	   write(ifout4,'(a,/)')'    I    Dist    Ie    M   Res    Wgt'
         do m=1,nn
c*	      gf 27/09/2018
            amagv=amg+bmg*aint0v(m)
c*
	      if(ammc.gt.10.d0)amagv=0.d0
            write(ifout4,'(1x,f5.1,f7.1,f6.2,f5.2,f6.2,f7.3)') 
     1      fintv(m),raiwv(m),aint0v(m),amagv,amagv-ammc,wgt(m)
         enddo
      endif
100   return
      end
...
c***********************************************************************
     ...
     1angle,stda,prayl,pkuip,ndat,ierr,nmin,ndecr,iout,locali,ifout4)
c
...
c
      ...
      ii=0
      if(iout.ge.2)then
         write(ifout4,'(/,1x,a)')'  DI     Dist    N   Azim'
      endif
      do i=1,ndecr
c
...
c
         if(iout.ge.2)then
            ...
            if(az.lt.200.d0)then
               write(ifout4,'(1x,f5.1,1x,f7.1,1x,i4,1x,f6.1)')
     1         (i-1)*.5,dis(i),nnn(i),az
            else
               write(ifout4,'(1x,f5.1,1x,f7.1,1x,i4)')
     1         (i-1)*.5,dis(i),nnn(i)
            endif
         endif
...
c
            ...
            if(iout.ge.2)then
               ...
               write(ifout4,'(1x,f6.1,1x,f3.1,1x,f6.1,1x,f7.4,
...
c*SUB MATRIXWR  ********************************************************
c***********************************************************************
      subroutine matrixwr(nincin,ninc,ninc1,lab,lab1,trix,ifout4)
c
      implicit real*8 (a-h,o-z)
      parameter(lp=7)
      common/metlik/met
...
c
      if(nincin.ge.3)then
         write(ifout4,'(7a11)') (lab(j),j=1,ninc),
     1   (lab(7),j=ninc,ninc1-1)
         do k=1,ninc
            write(ifout4,'(a5,7(1x,g10.4))')lab1(k),
     1      (trix(k,j),j=1,ninc),(trix(k,ninc1),j=ninc,ninc1-1)
         enddo
         if(met.eq.1)then
            write(ifout4,'(a5,7(1x,g10.4))')lab1(7),
     1      (trix(ninc1,j),j=1,ninc),(trix(ninc1,ninc1),j=ninc,ninc1-1)
         endif
      else
         if(nincin.eq.1)then
            write(ifout4,'(7a11)')lab(1),lab(2),
     1      (lab(7),j=ninc,ninc1-1)
         else 
            write(ifout4,'(7a11)')lab(1),lab(2),lab(4),
     1      (lab(7),j=ninc,ninc1-1)
         endif
         do k=1,ninc-1
            write(ifout4,'(a5,7(1x,g10.4))')lab1(k),
     1      (trix(k,j),j=1,ninc),(trix(k,ninc1),j=ninc,ninc1-1)
         enddo
         write(ifout4,'(a5,7(1x,g10.4))')lab1(4),
...
c
      ...
      sinaz=dcos(alatx*dtor)*dsin((alonx-alonp)*dtor)/dsin(dist)
      if(dsin(alatx*dtor).ge.dcos(dist)*dsin(alatp*dtor))then
         azimu1=dasin(min(1.d0,max(-1.d0,sinaz)))/dtor
      else
...
c*SUB BOX     **********************************************************
c***********************************************************************
      subroutine box(xlatem,xlonem,angle,adip,al,w,xlav,xlov)
c
c	Compute vertices of the rectangle representing the source
c
c	NOTE: cos45 indicate the 45 degrees immersion of the fault
c
      implicit real*8 (a-h,o-z)
      dimension xlav(*),xlov(*)
      data dtor/ 1.7453292519943296d-2/,gradla/111.195d0/,
	1cos45/0.7071067811865475d0/
c
      ang=(90.d0-angle)*dtor
	if(adip.gt.998.d0)then
	   dip=cos45
	else if(adip.eq.45.d0)then
	   dip=cos45
	else
c	   strike-slip fault -> 80 degrees
	   dip=min(adip,80.d0)
	   dip=(dip)*dtor
	endif
      axx=xlonem
      ayy=xlatem
      gradlo=gradla*dcos(xlatem*dtor)
      dxa=(al/2.d0*dcos(ang))/gradlo
      dya=(al/2.d0*dsin(ang))/gradla
c	 dxb=(w*cos45/2.d0*dsin(ang))/gradlo
c	 dyb=(w*cos45/2.d0*dcos(ang))/gradla
      dxb=(w*dip/2.d0*dsin(ang))/gradlo
      dyb=(w*dip/2.d0*dcos(ang))/gradla
      xlav(1)=ayy-dya+dyb
      xlov(1)=axx-dxa-dxb
      xlav(2)=ayy+dya+dyb
      xlov(2)=axx+dxa-dxb
...
c*SUB BOX plane   ******************************************************
c***********************************************************************
      subroutine boxplane(xlatem,xlonem,angle,adip,al,w,xlav,xlov)
c
c	Compute vertices of the rectangle representing the source plane (without immersion)
c
      implicit real*8 (a-h,o-z)
      dimension xlav(*),xlov(*)
      data dtor/ 1.7453292519943296d-2/,gradla/111.195d0/,
...
c	   strike-slip fault -> 80 degrees
      ...
      dxa=(al/2.d0*dcos(ang))/gradlo
      dya=(al/2.d0*dsin(ang))/gradla
c	 dxb=(w/2.d0*dsin(ang))/gradlo
c	 dyb=(w/2.d0*dcos(ang))/gradla
c	 dxb=(w*cos45/2.d0*dsin(ang))/gradlo
c	 dyb=(w*cos45/2.d0*dcos(ang))/gradla
      dxb=(w*dip/2.d0*dsin(ang))/gradlo
      dyb=(w*dip/2.d0*dcos(ang))/gradla
